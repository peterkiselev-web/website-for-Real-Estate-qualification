/* GENERATED FILE. Edit lib/communities.js and run `npm run sync`. */
/**
...
(function (root, factory) {
  ...
  /** What liking each card says about the area they should be looking in. */
  const CARD_SIGNALS = {
    'd1-marina': { waterfront: 3, highRise: 2, buzz: 2, central: 2, seaView: 2, metro: 1, dining: 2 },
    'd1-downtown': { central: 3, highRise: 3, skylineView: 3, metro: 2, buzz: 2, dining: 2 },
    'd1-community': { gated: 3, family: 3, quiet: 2, garage: 2, lowRise: 2, schools: 2, privatePool: 1 },
    'd1-beach': { beach: 3, prime: 3, waterfront: 3, privatePool: 2, seaView: 3, staffRoom: 1 },
    'd1-golf': { golf: 3, gated: 2, quiet: 3, family: 2, garage: 2, privatePool: 2, lowRise: 2 },
    'd1-offplan': { offplan: 3, newBuild: 3, yield: 1 },

    'a-seaview': { seaView: 3, waterfront: 2, beach: 1 },
    'a-highfloor': { highRise: 3, skylineView: 2, central: 1 },
    'green-balcony': { highRise: 1, communityPool: 1 },
    'green-lawn': { family: 2, lowRise: 2, garage: 1, park: 1, privatePool: 1 },
    'green-paved': { lowRise: 1, newBuild: 1 },
    'green-courtyard': { quiet: 2, established: 1, lowRise: 1 },
    'green-roof': { buzz: 1, dining: 1, highRise: 1, skylineView: 1 },
    'layout-open': { newBuild: 2 },
    'layout-separate': { family: 1, established: 1, majlis: 1 },
    'layout-atrium': { prime: 1, newBuild: 1 },
    'layout-office': { quiet: 1 },
    'layout-loft': { lowRise: 1 },
    'v-majlis': { majlis: 3, family: 2, lowRise: 1 },
    'v-staff': { staffRoom: 3, lowRise: 1, garage: 1 },

    'pool-community': { communityPool: 3, highRise: 1 },
    'pool-private': { privatePool: 3, lowRise: 1 },
    'pool-plunge': { privatePool: 2, lowRise: 1, quiet: 1 },
    'pool-none': { communityPool: 1 },

    'extra-garage': { garage: 3, lowRise: 1 },
    'extra-station': { metro: 3, central: 1 },
    'extra-gated': { gated: 3 },
    'extra-gym': { communityPool: 2, newBuild: 1 },
  };

  /** A pass is information too, just weaker and negative. */
  ...
  /** Which kind of home they are actually shopping for. */
  function kindFor(lead) {
    const branch = lead.branch;
    if (branch === 'villa' || branch === 'golf' || branch === 'prime') return 'villa';
    if (branch === 'apartment' || branch === 'offplan') return 'apartment';
    const yes = (lead.swipes || []).filter((s) => s.dir === 'y').map((s) => s.id);
    if (yes.includes('d1-community') || yes.includes('d1-golf') || yes.includes('d1-beach')) return 'villa';
    if (yes.includes('d1-marina') || yes.includes('d1-downtown') || yes.includes('d1-offplan')) return 'apartment';
    return 'any';
  }
  ...
  function budgetMaxFor(lead) {
    const band = Qualify && Qualify.option ? Qualify.option('budget', (lead.answers || {}).budget) : null;
    if (!band) return null;
   ...
   */
  function pickTiebreakers(lead, limit) {
    const answered = lead.tiebreaks || {};
    const signals = signalsFor(lead);
    const ranked = match(lead, { signals: signals, limit: 6 });
